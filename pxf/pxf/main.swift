    ...
    class AppDelegate: NSObject, NSApplicationDelegate, NSWindowDelegate, DropViewDelegate {
        ...
        private func getWatermarkDirectoryURL() -> URL {
            ...
            if !fileManager.fileExists(atPath: bundledDest.path) {
                if let data = Data(base64Encoded: bundledWatermarkBase64, options: .ignoreUnknownCharacters) {
                    ...
                    print("ERROR: bundledWatermarkBase64 decode returned nil")
                }
            }
            return wmDir
        }
        ...
        func presentOfferCodeRedemption() {
            ...
            if #available(macOS 15.0, *) {
                let state = OfferCodeState()
                state.onComplete = { [weak self] success in
                    guard let self = self else { return }
                    Task { @MainActor in
                        let entitled = success ? (await self.storeManager?.checkEntitlement() ?? false) : false
                        if entitled {
                            self.isPremiumUnlocked = true
                            self.applyPremiumRestrictions()
                            let done = NSAlert()
                ...
                let view = OfferCodeRedemptionView(state: state)
                ...
                // Fallback for macOS < 15: open App Store redemption page
    ...
    // Find the existing encodeWithAVFoundation function and replace it entirely with this.
        ...
        private func encodeWithAVFoundation(
            ...
            let watermarkCIImage: CIImage? = watermarkImageURL != nil ? CIImage(contentsOf: watermarkImageURL!) : nil
            ...
            asset.loadValuesAsynchronously(forKeys: ["tracks", "duration"]) { [weak self] in
                ...
                let capturedOutputWidth = outputWidth
                let capturedOutputHeight = outputHeight
                let videoComposition = AVMutableVideoComposition(asset: asset, applyingCIFiltersWithHandler: { [weak self] request in
                    ...
                    if let wmImage = watermarkCIImage {
                    ...
                    // 4. Scale to output size (half-size if requested)
                    let sourceExtent = request.sourceImage.extent
                    let scaleX = CGFloat(capturedOutputWidth) / sourceExtent.width
                    let scaleY = CGFloat(capturedOutputHeight) / sourceExtent.height
                    if abs(scaleX - 1.0) > 0.001 || abs(scaleY - 1.0) > 0.001 {
                        image = image.transformed(by: CGAffineTransform(scaleX: scaleX, y: scaleY))
                    }
                    let finalImage = image.cropped(to: CGRect(x: 0, y: 0, width: CGFloat(capturedOutputWidth), height: CGFloat(capturedOutputHeight)))
